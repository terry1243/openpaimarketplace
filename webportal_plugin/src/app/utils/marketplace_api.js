...
export async function getApprovedItems() {
  const url = `${MARKETPLACE_API_URL}/api/items?status=approved`;
...
export async function getPendingItems() {
  const url = `${MARKETPLACE_API_URL}/api/items?status=pending`;
  const res = await fetch(url);
  if (res.ok) {
    const items = await res.json();
    // order by updateDate
...
export async function deleteItem(itemId) {
  const url = `${MARKETPLACE_API_URL}/api/items/${itemId}`;
  const res = await fetch(url, {
    method: 'DELETE',
  });
  if (res.ok) {
...
export async function createMarketItem(marketItem) {
  const url = `${MARKETPLACE_API_URL}/api/items`;
  const res = await fetch(url, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
...
export async function approveItem(itemId) {
  const url = `${MARKETPLACE_API_URL}/api/items/${itemId}/status`;
  const res = await fetch(url, {
    method: 'Put',
    headers: {
...
export async function increaseSubmits(itemId) {
  const url = `${MARKETPLACE_API_URL}/api/items/${itemId}/submits`;
  const res = await fetch(url, {
    method: 'Put',
  });
  if (res.ok) {
...
export async function ensureUser(user) {
  const url = `${MARKETPLACE_API_URL}/api/users`;
  const res = await fetch(url, {
    method: 'POST',
    headers: {
      'Content-Type': 'text/plain',
...
export async function getStarStatus(username, itemId) {
  const url = `${MARKETPLACE_API_URL}/api/users/${username}/starItems/${itemId}`;
  const res = await fetch(url);
  if (res.ok) {
    return true;
